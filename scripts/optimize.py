...
class GridOptimizerExtreme:
    ...
    def optimize(
        ...
        print(
            f"   Grid Ratio:    {min(grid_ratio_range):.3f} - {max(grid_ratio_range):.3f}"
        )
        print(f"   Leverage:      {min(leverage_range)}x - {max(leverage_range)}x")
        print(
            f"   Max Position:  {min(max_position_range):.2f} - {max(max_position_range):.2f}"
        )
        print()

        # Teste chaque combo avec progress bar
        for grid_size, grid_ratio, leverage, max_pos in tqdm(
            combos, desc="🔄 Optimizing"
        ):
            config = {
        ...
        results_df = results_df.sort_values("sol_final", ascending=False).reset_index(
            drop=True
        )
    ...
    def print_top_results(self, results_df: pd.DataFrame, top_n: int = 10):
        ...
        if len(survivors) == 0:
            ...
            print(
                f"✅ {len(survivors)}/{len(results_df)} configurations ont survécu ({len(survivors)/len(results_df)*100:.1f}%)"
            )
        ...
        for idx, row in top_configs.iterrows():
            ...
            print(
                f"      Grid Ratio:    {row['grid_ratio']:>6.3f} ({row['grid_ratio']*100:.1f}% spacing)"
            )
            print(f"      Leverage:      {row['leverage']:>6.1f}x")
            print(
                f"      Max Position:  {row['max_position']:>6.2f} ({row['max_position']*100:.0f}%)"
            )

            print(f"   💰 PERFORMANCE:")
            print(
                f"      SOL: {row['sol_initial']:.4f} → {row['sol_final']:.4f} ({row['sol_change_pct']:+.1f}%)"
            )
            print(
                f"      USD: ${self.initial_capital:.0f} → ${row['value_usd_final']:.2f} ({row['return_usd_pct']:+.1f}%)"
            )
        ...
        print(
            f"   Survivors:                 {len(survivors):,} ({len(survivors)/len(results_df)*100:.1f}%)"
        )
        print(
            f"   Liquidations:              {len(results_df[results_df['liquidated']]):,}"
        )

        if len(survivors) > 0:
            print(f"\n   SOL Final:")
            print(
                f"      Median:                {survivors['sol_final'].median():.4f} SOL"
            )
            print(
                f"      Mean:                  {survivors['sol_final'].mean():.4f} SOL"
            )
            print(
                f"      Best:                  {survivors['sol_final'].max():.4f} SOL"
            )
            print(
                f"      Worst (survivor):      {survivors['sol_final'].min():.4f} SOL"
            )

            print(f"\n   USD Return:")
            print(
                f"      Median:                {survivors['return_usd_pct'].median():+.1f}%"
            )
            print(
                f"      Mean:                  {survivors['return_usd_pct'].mean():+.1f}%"
            )
            print(
                f"      Best:                  {survivors['return_usd_pct'].max():+.1f}%"
            )
            print(
                f"      Worst (survivor):      {survivors['return_usd_pct'].min():+.1f}%"
            )
...
if __name__ == "__main__":
    ...
    print(
        f"   Prix: ${data['close'].iloc[0]:.2f} → ${data['close'].iloc[-1]:.2f} ({((data['close'].iloc[-1]/data['close'].iloc[0])-1)*100:.1f}%)"
    )
    print()
