...
def load_data(filepath: str) -> pd.DataFrame:
    ...
    logging.info(
        f"First price: ${data['close'].iloc[0]:.2f}, Last price: ${data['close'].iloc[-1]:.2f}"
    )
...
def plot_performance(results_df: pd.DataFrame, bot: GridBotV3, save_path: str = None):
    ...
    fig.suptitle(
        "Grid Bot Performance Analysis - SOL Focused", fontsize=16, fontweight="bold"
    )
    ...
    ax.axhline(
        bot.initial_sol, color="gray", linestyle="--", alpha=0.5, label="Initial"
    )
    ...
    ax = axes[0, 1]
    ax.plot(
        results_df.index, results_df["portfolio_value_usd"], linewidth=2, color="blue"
    )
    ax.axhline(
        bot.initial_capital, color="gray", linestyle="--", alpha=0.5, label="Initial"
    )
    ...
    ax = axes[1, 1]
    ax.plot(
        results_df.index, results_df["active_positions"], linewidth=2, color="purple"
    )
...
def run_leverage_frontier_analysis(
    data: pd.DataFrame, base_config: dict, leverage_range: list
) -> pd.DataFrame:
    """
...
def plot_risk_frontier(frontier_df: pd.DataFrame, save_path: str = None):
    ...
    fig.suptitle(
        "Risk Frontier Analysis - Leverage Impact", fontsize=16, fontweight="bold"
    )

    # 1. SOL Final vs Leverage
    ax = axes[0, 0]
    ax.plot(
        frontier_df["leverage"],
        frontier_df["sol_final"],
        marker="o",
        linewidth=2,
        markersize=8,
        color="green",
    ...
    ax = axes[1, 1]
    scatter = ax.scatter(
        frontier_df["max_drawdown"],
        frontier_df["sol_change_pct"],
        c=frontier_df["leverage"],
        s=200,
        cmap="viridis",
        edgecolors="black",
        linewidth=1.5,
...
def main():
    ...
    print(
        f"   Buy & Hold:  {(buy_hold.iloc[-1]/base_config['initial_capital']-1)*100:+.2f}%"
    )
    print(
        f"   Sell & Hold: {(sell_hold.iloc[-1]/base_config['initial_capital']-1)*100:+.2f}%"
    )
